SET @progress_status_check := (
    ...
    WHERE tc.CONSTRAINT_SCHEMA = DATABASE()
      ...
      AND cc.CHECK_CLAUSE NOT LIKE '%STALE_CONFIRMATION%'
    LIMIT 1
);
...
ALTER TABLE `${tablePrefix}progress_operations`
    ADD CONSTRAINT `chk_progress_operations_status_v3`
    CHECK (`status` IN ('PENDING', 'SUCCESS', 'NO_POINTS_AWARDED', 'CANCELLED', 'INSUFFICIENT_FUNDS', 'BALANCE_LIMIT_EXCEEDED', 'STALE_CONFIRMATION'));
